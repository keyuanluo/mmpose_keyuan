...
def main():
    # 指定输入和输出的根目录路径
    input_root_dir = '/media/robert/4TB-SSD/watchped_dataset - 副本/Box_changed_deleted_same_image_name'   # 请将此路径替换为您的输入大文件夹路径
    output_root_dir = '/media/robert/4TB-SSD/watchped_dataset - 副本/Pose_changed_deleted_same_image_name' # 请将此路径替换为您的输出大文件夹路径
    ...
    for root, dirs, files in os.walk(input_root_dir):
        ...
        for file in files:
            ...
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff')):
                ...
                out_file = os.path.join(output_subdir, output_filename)
                ...
                if results.pred_instances is None or len(results.pred_instances) == 0:
                    ...
                    for idx, person_keypoints in enumerate(keypoints):
                        ...
                        data_row = {
                            'image_name': output_filename,  # 修改此处，仅包含文件名
                            'person_id': idx + 1
                        }
        ...
        # 如果当前目录包含图像且有关键点数据，则保存对应的 Excel 文件
        if has_image and data_list:
            df = pd.DataFrame(data_list)
            excel_file = os.path.join(output_subdir, 'keypoints.xlsx')
            ...
            print_log(
                f'在目录 {root} 中没有检测到人体关键点，未生成 Excel 文件。',
                logger='current',
                level=logging.INFO)
